...
const CONFIG = {
  ...
  DEFAULT_CENTER: { lat: 36.5, lng: 127.8 },
  DEFAULT_LEVEL: 13,
};
...
const AppState = {
  ...
  currentRegion: null,
  filterInstruments: [],
  mapClickListener: null, // 모달 지도 선택 이벤트 핸들러 참조
};
...
const APIService = {
  ...
  async fetchStores(region, instruments, keyword) {
    const params = new URLSearchParams();
    
    // 키워드가 있으면 키워드 검색, 없으면 필터 검색
    if (keyword) {
      params.append("keyword", keyword);
    } else {
      if (region) params.append("region", region);
      if (instruments && instruments.length === 1) params.append("instrument", instruments[0]);
    }

    return await this.request(`${CONFIG.API_BASE_URL}?${params.toString()}`);
  },
  ...
  async fetchStoreReviews(storeId) {
    return await this.request(`/api/stores/${storeId}/reviews`);
  },

  async fetchStoreDetail(storeId) {
    return await this.request(`/api/stores/${storeId}/detail`);
  }
};
...
const UIManager = {
  ...
  reviewPanel: {
    ...
    renderData(data) {
      ...
      DOM.aiSummary.innerHTML = (data.aiSummary && data.aiSummary.length > 0)
        ? `✅ AI 리뷰 요약: ${data.aiSummary.join(" | ")}`
        : `✅ AI 리뷰 요약: 요약 데이터가 없습니다.`;
      
      UIManager.renderReviewItems(data.reviews);
    },
  ...
  detailPanel: {
    async show(store) {
      ...
      this.resetContent(store.name);

      try {
        const data = await APIService.fetchStoreDetail(store.id);
        this.renderData(data);
      } catch (e) {
        DOM.detailAddress.innerText = "정보를 불러올 수 없습니다.";
        DOM.instrumentList.innerHTML = '<p style="padding:10px; color:red;">데이터 로딩 실패</p>';
      }
    },

    resetContent(name) {
      DOM.detailStoreName.innerText = name;
      DOM.detailAddress.innerText = "정보 불러오는 중...";
      DOM.detailPhone.innerText = "";
      DOM.detailHours.innerHTML = "";
      DOM.detailDescription.innerText = "";
      DOM.instrumentList.innerHTML = '<p style="padding:10px; color:#888;">상품 목록 로딩 중...</p>';
    },

    renderData(data) {
      DOM.detailAddress.innerText = data.address || "주소 정보 없음";
      DOM.detailPhone.innerText = data.phoneNumber || "전화번호 없음";
      DOM.detailHours.innerHTML = data.operatingHours 
           ? `${data.operatingHours} <span class="closed-day">(휴무일 정보 확인)</span>` 
           : "운영 시간 정보 없음";
      DOM.detailDescription.innerText = data.description || "등록된 매장 소개가 없습니다.";
              ...
              const itemHtml = `
                  <div class="instrument-item">
                      <div class="instrument-img-box">
                          <img src="${prod.imageUrl || 'https://via.placeholder.com/150?text=No+Image'}" alt="${prod.name}">
                      </div>
                      <div class="inst-info">
                          <p class="inst-name">${prod.name}</p>
                          <p class="inst-price">${formattedPrice}</p>
                      </div>
                  </div>`;
              DOM.instrumentList.insertAdjacentHTML('beforeend', itemHtml);
          });
      } else {
          DOM.instrumentList.innerHTML = '<p style="padding:10px; color:#999; font-size:13px;">등록된 상품이 없습니다.</p>';
      }
    },
  ...
  updateAuthUI(user) {
    if (user) {
      let menuHtml = `<span style="color:#333; font-weight:bold;">${user.name}님</span>`;
      if (user.role === "BUSINESS") {
         menuHtml += `<span onclick="location.href='manage-store.html'" style="color:#42d29e; font-weight:bold; cursor:pointer;">매장 관리</span>`;
      }
      menuHtml += `<span onclick="APIService.logout()">로그아웃</span><span>마이페이지</span>`;
      
      DOM.userMenu.innerHTML = menuHtml;
      if(DOM.registerBtn) DOM.registerBtn.style.display = (user.role === "BUSINESS") ? "block" : "none";
    } else {
...
const Controller = {
  ...
  handleSearchResults(stores) {
    ...
    if (results.length > 0) {
       // 첫 번째 결과로 이동 (검색 편의성)
       MapManager.move(results[0].lat, results[0].lng, 8);
    }

    results.forEach((store, index) => {
      const marker = MapManager.addMarker(store.lat, store.lng, store.name);
      kakao.maps.event.addListener(marker, 'click', () => this.handleStoreClick(store, index));
    });
  ...
  async executeFilterSearch() {
    ...
    try {
      const data = await APIService.fetchStores(AppState.currentRegion, AppState.filterInstruments, null);
  ...
  async executeKeywordSearch() {
    ...
    // UI 및 상태 초기화
    DOM.resultTitle.innerText = `'${keyword}' 검색 결과`;
    DOM.storeList.innerHTML = '<p class="result-placeholder">검색 중...</p>';
    AppState.currentRegion = null;
    AppState.filterInstruments = [];
    DOM.regionBtn.innerHTML = `지역 선택 <span>▼</span>`;

    try {
      const data = await APIService.fetchStores(null, null, keyword);
      this.handleSearchResults(data);
    } catch (error) {
      console.error(error);
      DOM.storeList.innerHTML = '<p class="result-placeholder" style="color:red;">검색 실패</p>';
    }
  },
  ...
  setupRegionDropdown() {
    ...
    UIManager.createMenu(DOM.regionDropdown, regionListData, {
      ...
      onItemClick: (guName, group) => {
        ...
        this.handleRegionSelect(guName, coords);
  ...
  bindEvents() {
    ...
    // Modal
    if(DOM.registerBtn) DOM.registerBtn.addEventListener("click", UIManager.modal.show);
    ...
    DOM.form.mapSelectBtn.addEventListener("click", () => {
      ...
      AppState.mapClickListener = (mouseEvent) => {
    ...
    // Submit
    DOM.form.submitBtn.addEventListener("click", () => this.handleRegisterSubmit());

    // 검색 이벤트 바인딩
    DOM.searchBtn.addEventListener("click", () => this.executeKeywordSearch());
    DOM.searchInput.addEventListener("keypress", (e) => {
        if (e.key === "Enter") this.executeKeywordSearch();
    });
  },

  async initApp() {
    MapManager.init();
    UIManager.detailPanel.hide();
